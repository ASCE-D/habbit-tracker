...
import { createHabit, editHabit } from "@/actions/habit";

//@ts-ignore
export function EditHabitModal({ onClose,habit
}) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const result = await editHabit(dataToSend);
      if (result.success) {
          ...
          <form onSubmit={handleSubmit}>
            ...
            <CardFooter className="flex justify-between">
              ...
              <Button type="submit" variant={"orange"}>
                <PlusIcon className="mr-2 h-4 w-4" /> Edit Habit
              </Button>
            </CardFooter>
          </form>
        </Card>
